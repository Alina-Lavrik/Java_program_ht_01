...
public class Robot3 {
    ...
    static {
        defaultIndex = 1;
        names = new ArrayList<String>();  // коллекцию инициализируем как пустую коллекцию
    }
     /** Уровень робота */
     private int level;
     ...
     private State state;    // новое поле состояние, либо робот включен либо выключен
    ...
    public Robot3(String name, int level) {
        ...
        if ((name.isEmpty()     // если имя робота будет пстым
                || Character.isDigit(name.charAt(0)))   // или первый символ будет являться цифрой 
                || Robot3.names.indexOf(name) != -1)           // или такое имя было задано ранее
        // в этом случае придумаем дефолтное имя
        {
            this.name =
        ...
        this.level = level;     // производим инциализацию начального уровня
        this.state = State.Off; // проводим инициализацию нач сост - робот выключен
