...
/**Указываем тип T и далее при созд команты можно будет указать какими типами наполнить команду */
class Team<T> {
    private String name;
    ...
    /**Можно добавить участника исключительно с типом Т */
    public void add(T p) {
        programmers.add(p);
    }
...
class Olympiad<T> {
    /**Олимпиада будет проходить только между командами кот имеют один и тот же тип */
    Team<T> team1, team2;
    Random r = new Random();
